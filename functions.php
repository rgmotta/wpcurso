...
add_theme_support('custom-background'); //fundos
add_theme_support('custom-header'); // cabeçalho
add_theme_support('post-thumbnails'); // miniaturas
add_theme_support('post-formats', array('video', 'image')); // formatos dos posts
...
function num_itens_blog( $query ){
	if( is_admin() || ! $query->is_main_query() ) // verificar se estamos dentro da página de administração
	return;
